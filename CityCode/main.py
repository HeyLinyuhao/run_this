...
def main(gpu,  ngpus_per_node, config, args):
    ...
    if args.ddp:
        ...
        if args.gcloud:
            if dist.get_rank() <= 0:
                logger.info("downloading city dataset ...")
                download_city_unzip(data_dir="./city_scape", prefix="yh/exercise_4/")
                logger.info("downloading resnet {} pretrained checkpoint ...".format(str(args.backbone)))
                download_checkpoint("CityCode/Model/Deeplabv3_plus/Backbones/pretrained/",
    ...
    supervised_loader = City(config['train_supervised'], ddp_training=args.ddp, dgx=args.dgx)
    unsupervised_loader = City(config['train_unsupervised'], ddp_training=args.ddp, dgx=args.dgx)

    val_loader = City(config['val_loader'], dgx=args.dgx)
    ...
    if config['model']['sup_loss'] == 'CE':
        sup_loss = CE_loss
    elif config['model']['sup_loss'] == 'OHEM':
        sup_loss = ProbOhemCrossEntropy2d(ignore_label=255, thresh=0.7,
                                          min_kept=50000*args.batch_size, use_weight=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--labeled_examples', default=372, type=int)
    ...
    if args.architecture == "deeplabv3+":
        config = json.load(open("CityCode/configs/config_deeplab_v3+.json"))
    else:
        raise NotImplementedError
