...
class upsample(nn.Module):
    def __init__(self, in_channels, out_channels, data_shape,
                 norm_act=nn.BatchNorm2d,cls_specific_recon=False, num_semantic_cls=None):
        super(upsample, self).__init__()
        self.data_shape = data_shape

        self.cls_specific_recon = cls_specific_recon
        self.num_semantic_cls = num_semantic_cls


        self.classifier = nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=True)

        if self.cls_specific_recon:
            self.classifier_list = []
            for _ in range(self.num_semantic_cls):
    ...
    def forward(self, x, data_shape=None):
        ...
        if self.cls_specific_recon:
            logits_list = []
            for i in range(self.num_semantic_cls):
                logits_list.append(F.interpolate(self.classifier[i](f), size=(h, w), mode='bilinear', align_corners=True))
            # print('______________')
            # print(len(logits_list))
            res = {"pred": logits_list}
        else:
            pred = self.classifier(f)
            res = {"pred":F.interpolate(pred, size=(h, w), mode='bilinear', align_corners=True)}
...
class DecoderNetwork(nn.Module):
    def __init__(self, num_classes,
                 data_shape,
                 conv_in_ch=256,cls_specific_recon=False, num_semantic_cls=None):

        super(DecoderNetwork, self).__init__()
        self.upsample = upsample(conv_in_ch, num_classes, norm_act=torch.nn.BatchNorm2d,
                                 data_shape=data_shape,cls_specific_recon=cls_specific_recon, num_semantic_cls=num_semantic_cls)
        self.business_layer = []
