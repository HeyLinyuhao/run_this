...
class Trainer(BaseTrainer):
    ...
    def cut_mix2(self, labeled_image=None, labeled_mask=None,
                unlabeled_image=None, unlabeled_mask=None):
        # for labelled image
        mix_labeled_image = labeled_image.clone()


        u_rand_index_0 = torch.randperm(labeled_image.size()[0])[:labeled_image.size()[0]].cuda()
        u_bbx1, u_bby1, u_bbx2, u_bby2 = self.rand_bbox_1(labeled_image.size(), lam=np.random.beta(4, 4))

        for i in range(0, mix_labeled_image.shape[0]):
            mix_labeled_image[i, :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]] = \
                labeled_image[u_rand_index_0[i], :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]]


        # for unlabelled image
        mix_unlabeled_image = unlabeled_image.clone()
        mix_unlabeled_target = unlabeled_mask.clone()
        u_rand_index = torch.randperm(unlabeled_image.size()[0])[:unlabeled_image.size()[0]].cuda()
        u_bbx1, u_bby1, u_bbx2, u_bby2 = self.rand_bbox_1(unlabeled_image.size(), lam=np.random.beta(4, 4))
        ...
        return labeled_image, labeled_mask, mix_labeled_image, mix_unlabeled_image, mix_unlabeled_target


    def predict_with_out_grad(self, image):
        with torch.no_grad():
            predict_target_ul1 = self.model.module.decoder1(self.model.module.encoder1(image),
                                                            data_shape=[image.shape[-2], image.shape[-1]])['pred']
    ...
    def _train_epoch(self, epoch, id):
        ...
        for batch_idx in tbar:
            ...
            if batch_idx == 0 or batch_idx == int(len(self.unsupervised_loader)/2):
                # fea = torch.mean(self.model.module.decoder_sa.upsample.last_conv(self.model.module.encoder_s(input_l)),dim = 1)
                if self.args.local_rank <= 0:
                    self.tensor_board.update_wandb_recons_image(images=outputs['lab_img'],recon=outputs['lab_rec_img'], img_number=min(self.args.batch_size, 4))
                    # self.tensor_board.update_wandb_recons_image2(images=outputs['unlab_img'],recon=outputs['unlab_rec_img'], img_number=min(self.args.batch_size, 4))
