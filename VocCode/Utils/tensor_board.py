...
class Tensorboard:
    ...
    def update_wandb_recons_image(self, images,
        ...
        wandb.log({"images_gt":  [wandb.Image(j, caption="id {}".format(str(i)))
                                    for i, j in enumerate(self.de_normalize( images[:img_number, :, :]))],
                    "global_step": self.current_step})
    ...
    def update_wandb_recons_image2(self, images,
                           recon,
                           img_number=4):

        # images = self.de_normalize(images)
        wandb.log({"images_gt2":  [wandb.Image(j, caption="id {}".format(str(i)))
                                    for i, j in enumerate(self.de_normalize( images[:img_number, :, :]))],
                    "global_step": self.current_step})

        wandb.log({"recovered2": [wandb.Image(j, caption="id {}".format(str(i)))
                                    for i, j in enumerate(self.de_normalize(recon[:img_number, :, :]))],
                   "global_step": self.current_step})

        # wandb.log({"feature_map": [wandb.Image(j, caption="id {}".format(str(i)))
        #                             for i, j in enumerate(fea[:img_number, :, :])],
        #            "global_step": self.current_step})
